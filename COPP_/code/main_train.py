...
def run(envt, ogp, alg):
    ...
    ACTION_LIST_increase = [1.5, 2.5]
    ACTION_LIST_reduce = [0.3, 0.6]
    ...
    for episode in range(EPISODES):
        ...
        while len(H) != 0:
            ...
            for update_agent_id, update_agent in enumerate(ILP_agents):
                ...
                if label == 0:
                    ...
                    if agent_new.now_node == agent_new.end_node:
                        #reduce_car_num += (agent_new.now_t - agent_new.appear_time)
            ...
            if label == 0 and envt.current_time - envt.last_time > envt.action_unit_time: # 每30秒做一次动作
                ...
                # 计算reward
                real_reward = 0.999*now_all_car_num_ + 0.001*envt.delta_time#*((abs(FIRST_CAR_NUM)+1)/(abs(FIRST_QUERY_TIME)+1))
                ...
                for edge_count, edge in enumerate(envt.use_edge):
                    ...
                    observation = envt.node_attr_index[edge[0]]+[envt.car_number_of_edge[(edge[0], edge[1])]]#[car_number_beishu, envt.car_number_of_edge[(edge[0], edge[1])], envt.Street_attr[(edge[0], edge[1])].Query_time_gap, envt.Street_attr[(edge[0], edge[1])].Query_state] #+ dic_one_hot_street_id[envt.Street_attr[(edge[0], edge[1])].Street_id]
                    '''
                    # 观测值标准化，第一回合只为了存数据
                    if episode > 0:
                ...
                for edge_count, edge in enumerate(envt.use_edge):
                    ...
                    # 存储每个街道的s,a,s_
                    if len(envt.temp_obs) != 0 and episode > 1: #and edge_count == 6:
