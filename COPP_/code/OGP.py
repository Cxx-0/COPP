        ...
        for agent in agents:
            if agent.now_node == agent.start_node:       
                ...
                while now_node != agent.end_node:
                    ...
                    #self.envt.L_other[now_node][adj_node].append((agent.pred_arrive_time, agent.pred_arrive_time+t_va_v))
        ...
        while len(PQ) != 0:
            ...
            for adj_node in self.envt.get_adj_node(v):
                ...
                for time_label in self.envt.L[v][adj_node]:    
                    ta = time_label[0]
                    tb = time_label[1]
                    #if ta <= To_time[v]+0.1 and tb >= To_time[v]-0.05:
                    if ta <= To_time[v] and tb >= To_time[v]:
                        flow += 1
                
                # 思路是如果当前街道一个点处于被查询街道端点上，则查询被查询街道车辆数
        ...
        while len(PQ) != 0:
            ...
            if v == agent.end_node:
                while v != agent.now_node:
                    ...
                    arrive_time.insert(0, To_time[v])
                    flow_num.insert(0, Flow_num[v])
                    #self.envt.L_other[int(pre_node[v])][v].append((To_time[int(pre_node[v])],To_time[v]))
                    v = pre_node[v]   
                break
        ...
        for agent in agents:
            if len(agent.temp_route) == 0:
                ...
                while now_node != agent.end_node:
                    ...
                    #self.envt.L_other[now_node][one_agent_final_action].append((agent.pred_arrive_time, agent.pred_arrive_time+t_va_v_min))
